...
> getJSON() 对指定的 URL 启动一个异步 HTTP 请求，然后在该请求待定时，它返回一个 Promise 对象。Promise 对象定义了 then() 实例方法。我们没有将回调函数直接传递给 getJSON()，而是将其传递给 then() 方法。当 HTTP 响应时，该响应的主体将解析为 JSON，并将所解析的结果值传递给我们传递给 then() 的函数。
...
> 在我们进一步讨论 Promises 之前，需要暂停定义一些术语。当我们不编程时，我们谈论的是人类的诺言，我们说“信守”或“打破”诺言。在讨论 JavaScript Promise 时，用“已兑现（fulfilled）”和“拒绝（rejected）”。想象一下，您已经调用了 Promise 的 then() 方法，并向其传递了两个回调函数。我们说，当调用第一个回调，那么 Promise 已兑现。我们说，当调用第二个回调，则 Promise 已被拒绝。 如果一个 Promise 既没有已兑现也不被拒绝，那么它就是待定（pending）。一旦 Promise 已兑现或拒绝，我们就说它已敲定（settled）。请注意，一个 Promise 永远不会同时已兑现和拒绝。Promise 一旦达成，就永远不会从已兑现变为拒绝，反之亦然。
...
> 记住我们在本节开始时如何定义 Promise：“Promise 是描述异步计算结果的对象。”重要的是要记住，Promise 不仅仅是注册在某些异步代码完成时运行的回调的抽象方式，它们还描述了异步代码的结果。如果异步代码正常运行（并且已兑现了 Promise），那么该结果实质上就是代码的返回值。而且，如果异步代码无法正常完成（并且 Promise 被拒绝），那么结果将是 Error 对象或其他代码（如果代码不是异步的话）可能会抛出的其他值。任何已敲定的 Promise 都有与其相关的值，并且该值不会改变。如果已兑现了 Promise，则该值是一个返回值，该值将传递给注册为 then() 第一个实参的回调函数。如果 Promise 被拒绝，则该值是某种错误，该错误会传递给使用 catch() 或 then() 的第二个实参注册的任何回调函数。
...
> 我希望对 Promise 术语保持精确的原因是 Promise 也可以解决。将已决议状态与已完成状态或已决议状态混淆是很容易的，但是两者都不完全相同。 理解已决议状态是深入了解Promise的关键之一，在下面讨论了Promise链之后，我将再次介绍它。 todo
...
> 在本章的前面，我们看到了 XMLHttpRequest 对象，该对象用于在JavaScript中发出HTTP请求。这个奇怪命名的对象具有一个旧的且笨拙的 API，并且在很大程度上已被较新的基于 Promise 的 Fetch API（§15.11.1）所取代。以最简单的形式，这个新的 HTTP API 只是函数 fetch()。给它传递一个 URL，然后它返回一个 Promise。当 HTTP 开始收到响应并且 HTTP 状态和标头可用时，将兑现该 promise：
...
> 当 fetch() 返回的 Promise 已兑现时，它将 Response 对象传递给传递给 then() 方法的函数。此 response 对象使您可以访问请求状态和标头，并且还定义了诸如 text() 和 json() 之类的方法，这些方法可以分别以文本和 JSON 的形式访问 Response 的正文。但是，尽管最初的 Promise 已兑现，但响应的主体可能尚未到达。因此，这些用于访问响应正文的 text() 和 json() 方法本身返回 Promise。 这是使用 fetch() 和 response.json() 方法获取 HTTP 响应正文的一种简单方法： 
...
> 有时，当一个 API 设计为使用这种方法链接时，只有一个对象，并且该对象的每个方法都返回该对象本身以便于链接。但是，这不是 Promise 的工作方式。当我们编写一系列的 .then() 调用时，我们并未在单个 Promise 对象上注册多个回调。而是，对 then() 方法的每次调用都会返回一个新的 Promise 对象。在传递给 then() 的函数完成之前，新的 Promise 对象不会已兑现。
...
> 2. 在第二行，我们调用 promise 1 的 then() 方法，并传递要在已兑现 promise 1 时要调用的 callback1 函数。then() 方法将回调函数存储在某个位置，然后返回一个新的 Promise。我们将在这一步返回的新 Promise 称为“promise 2”，并且说“task 2”在调用 callback1 时开始。
> 3. 在第三行，我们调用 promise 2 的 then() 方法，并在已兑现 promise 2 时传递要调用的 callback2 函数。这个 then() 方法会记住我们的回调并返回另一个 Promise。我们将说，“task 3”是在调用 callback2 时开始的。我们可以将最新的 Promise 称为“promise 3”，但是我们并不需要它的名称，因为我们根本不会使用它。
> 4. 最初执行表达式时，前三个步骤都是同步发生的。现在，在第 1 步中启动的 HTTP 请求通过 Internet 发送时，我们有了异步暂停。
> 5. 最终，HTTP 响应开始到达。fetch() 调用的异步部分将 HTTP 状态和标头包装在 Response 对象中，并以该 Response 对象作为值来已兑现 promise 1。
> 6. 已兑现 promise 1 后，其值（Response 对象）将传递到我们的 callback1() 函数，然后 task 2 开始。给定一个 Response 对象作为输入，此任务的工作是将响应主体作为 JSON 对象获得。
> 7. 假设 task 2 正常完成，并且能够解析 HTTP 响应的主体以生成 JSON 对象。此 JSON 对象用于已兑现 promise 2。
> 8. 已兑现 promise 2 的值在传递给 callback2() 函数时成为 task 3 的输入。现在，这第三项任务以某种未指定的方式向用户显示数据。当 task 3 完成时（假设它正常完成），则 promise 3 将被已兑现。但是，因为我们从未对 promise 3 做任何事情，所以当 Promise 兑现时，什么也没有发生，并且异步计算链到此结束。 
...
> 请记住，仅当前一阶段的回调引发错误时，才会调用传递给 catch() 的回调。如果该回调正常返回，则将跳过 catch() 回调，并且前一个回调的返回值将成为下一个 then() 回调的输入。还请记住，catch() 回调不仅用于报告错误，而且用于处理错误并从错误中恢复。将错误传递给 catch() 回调后，它将停止沿 Promise 链传播。catch() 回调可以引发新的错误，但是如果它正常返回，则该返回值用于解决和或或已兑现关联的 Promise，并且错误停止传播。
...
> Promise.all() 比以前描述的要灵活一些。输入数组可以包含 Promise 对象和非 Promise 值。如果数组的元素不是 Promise，则将其视为已已兑现的 Promise 的值，并简单地将其原样复制到输出数组中。
